...
public class Pawn extends ChessPiece{
	...
	public boolean[][] possibleMoves() {
		...
		if(getColor() == Color.WHITE) {
			...
			p.setValues(position.getRows() - 2, position.getColumn());
			Position p2 = new Position(position.getRows() - 1, position.getColumn());
			...
			if (position.getRows() == 4) {
				...
				// #specialmove en passant black right
				Position right = new Position(position.getRows(), position.getColumn() + 1);
				if (getBoard().positionExists(right) && isThereOpponentPiece(right)
						&& getBoard().piece(right) == chessMatch.getEnPassantVulnerable()) {
					mat[right.getRows() + 1][right.getColumn()] = true;
